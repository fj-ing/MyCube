...
void angle_calc()
{
  ...
  static float filtered_angleX = 0, filtered_angleY = 0;
  filtered_angleX = 0.9 * filtered_angleX + 0.1 * Acc_angleX;
  filtered_angleY = 0.9 * filtered_angleY + 0.1 * Acc_angleY;

  if (abs(AcX) < 2000 && abs(Acc_angleX) < 2.0 && abs(Acc_angleY) < 2.0 && !vertical_vertex && !vertical_edge)
  {
    robot_angleX = Acc_angleX;
    robot_angleY = Acc_angleY;
    vertical_vertex = true;
  }
  else if (abs(AcX) > 7000 && abs(AcX) < 10000 && abs(Acc_angleX) < 2.0 && !vertical_vertex && !vertical_edge)
  {
    robot_angleX = Acc_angleX;
    robot_angleY = Acc_angleY;
    vertical_edge = true;
